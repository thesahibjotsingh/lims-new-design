...
//
// PORTRAITS are rendered — but only a real one. `doctor.portrait` must be a photograph
// of that consultant, supplied by LIMS. A stock portrait captioned with a real doctor's
// name is an invented likeness of that person, so there is no placeholder portrait and
// no generic silhouette: a doctor with no supplied photograph gets the monogram layout,
// which is why both layouts below are built to look finished on their own.
//
// So the card is built to look complete with only what is real — name, qualifications,
...
export function DoctorCard({ doctor }: { doctor: Doctor }) {
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-brand-teal/10 bg-white shadow-sm transition-shadow hover:shadow-lg">
      {portrait && (
        /*
          Fixed aspect ratio rather than the image's own, so a row of cards keeps one
        ...
        */
        <div className="relative aspect-[4/5] w-full overflow-hidden bg-brand-mist max-md:aspect-[3/2]">
          {/*
            ...
            for nothing. Portraits are pre-sized by scripts/build_assets.py.
          */}
      ...
      <div className="flex flex-1 flex-col justify-between gap-5 p-6">
        <div className="space-y-3">
          <div className="flex items-start gap-3">
            {/*
              The monogram stays even when a portrait is present: it anchors the name
              row at both card sizes and keeps the two layouts recognisably the same
              component. Without a portrait it is the only likeness the card carries.
            */}
            <span
              aria-hidden="true"
